...
void AutoCompleterTextEdit::setCompleter(QCompleter* c) {
	...
	connect(comp, static_cast<void(QCompleter::*)(const QString&)>(&QCompleter::activated),
		this, &AutoCompleterTextEdit::insertCompletion);
}

QCompleter* AutoCompleterTextEdit::completer() const {
	return comp;
}
...
{
	// If the completer is visible it is responsible to handle the following events
	...
	if (e->text().isEmpty() || completionPrefix.length() < 3 || e->text().right(1) == "(") {
		comp->popup()->hide();
		return;
	}
