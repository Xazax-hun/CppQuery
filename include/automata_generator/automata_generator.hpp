...
{
  // check if we can compose Dest and Target in any way, i.e. Dest(Target) or Target(Dest)
...
struct TryComposeWithAll<Dest, Dest>
{
  typedef typename boost::mpl::vector<
      ComposabilityRule<Dest, Dest>
    >::type result;
...
struct AutomataImpl
{
  ...
  // gather all results for 'Pair', i.e. all functions that are composable with 'Pair'
  typedef typename TryComposeWithAll<Pair, Pair, Pairs...>::result partial;

  // merge the results for 'Pair' with the results of the elements of 'Pairs' (recurse)
  typedef typename boost::mpl::copy<
      typename AutomataImpl<Pairs...>::result,
      boost::mpl::back_inserter<partial>
    >::type result;
};

template <typename Pair>
struct AutomataImpl<Pair>
{
  typedef typename boost::mpl::vector<ComposabilityRule<Pair, Pair>>::type result;
};

// 'Pairs' is a pack of (decltype, name) pairs: one such pair represents a single function whose composability we analyze.
