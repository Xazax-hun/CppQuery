...
#define FUNCTION(x) std::tuple<decltype(x), FunctionPointer<decltype(x), &x>, MetaString<_S( #x )>>
...
template <typename F, typename G, int Par>
struct ComposabilityHelper
{
...
>
{
  ...
  static ComposeResult TryCompose(const std::string& f,
                                  const std::string& g,
                                  int parameter = 0)
  {
    // TODO: support arbitary number of arity
    const bool composable[5] = {
      ComposabilityHelper<F, G, 0>::value,
  ...
  static std::string GetInnerFunction(const std::string& f,
                                      int parameter = 0)
  {
    const bool composable[5] = {
      ComposabilityHelper<F, G, 0>::value,
