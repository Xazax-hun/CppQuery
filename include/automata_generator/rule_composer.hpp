...
template <class GetInstancePolicy, typename IsComposable, typename... IsComposables>
struct ComposedRules
{
  // Checks whether the functions represented by their names are composable.
  static bool CanCompose(const std::string& f,
                         const std::string& g,
                         int parameter = 0)
  {
    switch(IsComposable::TryCompose(f, g, parameter))
    {
      case RuleNotApplicable:
        return ComposedRules<GetInstancePolicy, IsComposables...>::CanCompose(f, g, parameter);
  ...
  static std::vector<std::string> GetComposables(const std::string& f,
                                                 int parameter = 0)
  {
    auto v = ComposedRules<GetInstancePolicy, IsComposables...>::GetComposables(f, parameter);
  ...
  static typename GetInstancePolicy::ResultType GetInstance(const std::string& f)
  {
    if (IsComposable::FunctionName::GetRuntimeString() != f)
      return ComposedRules<GetInstancePolicy, IsComposables...>::GetInstance(f);

    return GetInstancePolicy::template Create<typename IsComposable::ObjectType>();
  }
};
...
struct ComposedRules<GetInstancePolicy, IsComposable>
{
  ...
  {
    if (IsComposable::FunctionName::GetRuntimeString() != f)
      assert(false && IsComposable::FunctionName::GetRuntimeString().c_str() && " can not be instantiated");
